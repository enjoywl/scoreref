...
async function fetchMatches() {
  ...
  error.value = null;
  try {
    const res = await fetch(`/api/matches?date=${selectedDate.value}&status=${statusFilter.value}`);
...
function getStatus(s: number) {
  return STATUS_MAP[s] || { label: "未知", cls: "" };
}
    ...
    <div class="toolbar">
      <div class="toolbar-left">
        <el-select
          ...
          style="width: 110px"
        >
          <el-option label="按联赛" value="league" />
      ...
      <span class="count">共 {{ matches.length }} 场</span>
    </div>
        ...
        <div v-for="m in list" :key="m.mid" class="match-card">
          <div class="match-body">
            <div class="match-meta">
              ...
              <span :class="['match-status', getStatus(m.stat).cls]">{{ getMatchTime(m) }}</span>
              <span v-if="m.locn" class="venue">{{ m.locn }}</span>
            </div>
